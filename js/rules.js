...
const Rules = {
  ...
  moveBudget(u) {
    return u.webbed ? Math.max(1, Math.ceil(u.def.move / 2)) : u.def.move;
  },
  ...
  // there (walked, pushed, leapt). Water is the big one: anything that can't
  // fly is washed out of the fight.
  landOn(g, u) {
  ...
  armourOf(g, u) {
    const t = this.tile(g, u.r, u.c);
    return (u.def.armour || 0) + ((t && t.cover) || 0);
  },

  damageOf(g, atk, def, base) {
    const raw = base != null ? base : atk.def.atk;
  ...
  // Shove a unit directly away from `from` (dominant axis wins ties by row).
  // Hitting a wall, a rock or another unit stops it short for 1 bump damage;
  // landing in the pond takes it out of the fight entirely.
  push(g, u, from, dist) {
    if (u.def.heavy) { g.emit({ type: "resist", u }); return; }
    const dr = u.r - from.r, dc = u.c - from.c;
    let sr = 0, sc = 0;
    if (Math.abs(dr) >= Math.abs(dc)) sr = Math.sign(dr) || (u.r <= from.r ? -1 : 1);
    else sc = Math.sign(dc);
    if (!sr && !sc) sr = -1;
    let moved = 0;
    for (let i = 0; i < dist; i++) {
      const nr = u.r + sr, nc = u.c + sc;
      const t = this.tile(g, nr, nc);
      if (!t || t.solid || this.unitAt(g, nr, nc)) {
        this.damage(g, u, 1, from, "bump");    // thumped into something solid
        break;
      }
      u.r = nr; u.c = nc; moved++;
      if (t.water && !u.def.flier) break;      // landOn below washes them out
    }
    if (moved) {
      g.emit({ type: "shove", u });
      this.landOn(g, u);
  ...
  powerTargets(g, u) {
    ...
    switch (p.kind) {
      case "pull":
        ...
        return enemies.filter(o => this.dist(u, o) > 1 && this.dist(u, o) <= p.range);
      case "push":
        return enemies.filter(o => this.dist(u, o) <= p.range);
      case "blast":
        return enemies.filter(o => this.dist(u, o) === 1);
      case "heal":
        return friends.filter(o => this.dist(u, o) <= p.range && o.hp < o.maxHp);
      case "charge":
        // Straight lines only — the roll has to be readable before you commit.
        return enemies.filter(o => (o.r === u.r || o.c === u.c) && this.dist(u, o) <= p.range);
      default:
        return [];
    }
  },
    ...
    switch (p.kind) {
      ...
      case "blast": {
        ...
        for (const [dr, dc] of DIRS) {
          ...
          if (t && t.breakable) { this.setTile(g, r, c, "."); g.emit({ type: "rubble", r, c }); }
        }
        break;
      }
      case "heal":
  ...
  pullToward(g, u, to) {
    const dr = Math.sign(to.r - u.r), dc = Math.sign(to.c - u.c);
    // Move along the dominant axis so the yank is predictable.
    const [sr, sc] = Math.abs(to.r - u.r) >= Math.abs(to.c - u.c) ? [dr, 0] : [0, dc];
    const nr = u.r + sr, nc = u.c + sc;
    if (this.canEnd(g, u, nr, nc) || (this.tile(g, nr, nc) && this.tile(g, nr, nc).water && !this.unitAt(g, nr, nc))) {
      u.r = nr; u.c = nc;
      g.emit({ type: "shove", u });
      this.landOn(g, u);
    }
